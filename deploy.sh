...
set deploy_command [format {set -eu; install -d -o cerato -g cerato %s; git config --global --add safe.directory %s; cd %s; fresh_checkout=0; old_lock=$(sha256sum package-lock.json 2>/dev/null | awk '{print $1}' || true); if [ ! -d .git ]; then fresh_checkout=1; git init -q; git clean -fdx -e .env -e .env.*; fi; if git remote get-url origin >/dev/null 2>&1; then git remote set-url origin %s; else git remote add origin %s; fi; git fetch --quiet origin %s; git clean -fd -e .env -e .env.*; git checkout -q -B %s origin/%s; git reset --hard origin/%s; new_lock=$(sha256sum package-lock.json | awk '{print $1}'); if [ "$fresh_checkout" = 1 ] || [ "$old_lock" != "$new_lock" ] || [ ! -x node_modules/.bin/next ]; then runuser -u cerato -- npm ci; fi; chown -R cerato:cerato %s; runuser -u cerato -- npm run build; systemctl restart cerato-web; systemctl is-active --quiet cerato-web} $app_dir $app_dir $app_dir $repository $repository $branch $branch $branch $branch $app_dir]
